        ...
        <div class="app-shell" data-cloud-port="{settings.cloud_port}" data-debug-port="{settings.debug_port}" data-server-url="{settings.local_base_url}" data-cloud-base-url="{settings.local_base_url}" data-debug-base-url="{settings.debug_base_url}">
          <header class="topbar">
            ...
            <nav class="topnav" aria-label="Hauptnavigation">
              ...
              <button class="nav-button" data-view="setup" data-normal data-easy>Setup</button>
              <button class="nav-button" data-view="devices" data-normal data-easy>Radios</button>
              <button class="nav-button" data-view="health" data-normal>Status &amp; Diagnose</button>
              <button class="nav-button" data-view="controls" data-easy>Fernbedienung</button>
              <button class="nav-button" data-view="stations">Sender</button>
              <button class="nav-button" data-view="presets" data-normal data-easy>Presets</button>
              <button class="nav-button" data-view="multiroom" data-easy>Multiroom</button>
          ...
          <main class="workspace">
            ...
            <section class="view" id="view-devices">
              ...
              <section class="panel easy-hidden"><h3>Live-Capture Vergleich</h3><div id="device-live-comparison" class="event-list"></div><details><summary>Endpoint-Matrix</summary><pre id="device-live-comparison-output">Noch kein Vergleich geladen.</pre></details></section>
            </section>
            <section class="view" id="view-health">
              ...
              <section class="panel research-health-picker"><label>Radio<select id="health-device-select" required><option value="">Noch kein Radio vorhanden</option></select></label><div class="health-readback-note"><strong>Wiedergabe stammt vom Radio-Readback</strong><span>Provider, Stream, Metadaten und Reporting bleiben davon unabhängige Signale.</span></div></section>
              ...
              <div class="split research-health-primary">
                <section class="panel"><div class="panel-title-row"><h3>Wiedergabezustand</h3><span id="playback-health-badge" class="status-pill">UNKNOWN</span></div><div id="playback-health-details" class="event-list"></div></section>
              ...
              <div class="settings-grid">
                <section class="panel">
                  ...
                  <form id="device-settings-form" class="settings-form">
                    ...
                    <label>Anzeige am Radio<select name="station_art_mode"><option value="radio_symbol">Bose-Standardsymbol über Sendernamen anzeigen</option><option value="station_logo">Senderlogo über Sendernamen anzeigen</option><option value="no_station_logo">Kein Senderlogo</option></select><small>Die Auswahl wird lokal gespeichert. Bestehende Presets brauchen danach eine Synchronisierung mit Backup, Vorschau und Readback.</small></label>
            ...
            <section class="view" id="view-stations">
              ...
              <div class="panel"><table><thead><tr><th>Name</th><th>Stream</th><th>Image</th><th>Action</th></tr></thead><tbody id="stations-table"></tbody></table><pre id="station-play-output" data-i18n-static>Choose a radio and station to test playback.</pre></div>
              ...
              <div class="preset-builder">
                <section class="panel preset-workbench">
                  ...
                  <form id="preset-form">
                    ...
                    <p class="form-message" id="preset-form-status" role="status" aria-live="polite">Das Radio bestätigt jede Änderung per Readback.</p>
                  </form>
                </section>
              ...
              <div class="multiroom-grid clean-grid">
                <section class="panel focus-panel">
                  ...
                  <form id="multiroom-form" class="multiroom-form">
                    ...
                    <label>Sender beim Start (optional)<select id="multiroom-station" name="station_id"><option value="">Vorhandene Quelle verwenden</option></select><small>SoundTouch kann beim Gruppieren die zuletzt verwendete Quelle fortsetzen sowie Mute und Lautstärke selbst ändern. Die Vorschau zeigt deshalb alle aktuellen Werte.</small></label>
                    ...
                    <label class="toggle-line"><input id="multiroom-start-volumes-enabled" name="set_start_volumes" type="checkbox"> Lautstärken vor dem Start festlegen</label>
                    <div id="multiroom-start-volumes" class="start-volume-grid" hidden></div>
                  ...
                  <details class="single-room-remove lab-only"><summary>Experimental: Mitglied aus laufender Gruppe entfernen</summary><p class="muted-copy">Firmwareabhängige LAB-Funktion. Entfernt nur dieses Radio aus der aktuellen Zone. Das Radio bleibt in BASSWIESN gespeichert.</p><div id="multiroom-remove-device" class="radio-button-picker"></div><button class="command" id="multiroom-remove" type="button">Mitglied entfernen</button></details>
                </section>
                <section class="panel">
            ...
            <section class="view" id="view-system-settings">
              ...
              <div class="settings-grid">
                <section class="panel"><h3>WebGUI defaults</h3><form id="system-settings-form" class="settings-form"><label>Interface mode<select id="ui-mode-setting" name="ui_mode"><option value="easy">Easy Mode</option><option value="standard">Standard Mode</option><option value="lab">LAB Mode</option></select><small>Easy Mode changes only the interface; all stable backend functions remain available.</small></label><label>Sprache<select id="web-language-select" name="web_language"></select></label><input id="lab-mode" name="lab_mode" type="checkbox" hidden><label class="toggle-line"><input id="guided-hints" name="guided_hints" type="checkbox" checked>Guided hints</label><label>Safe Startup Volume<input id="safe-startup-volume" name="safe_startup_volume" type="number" min="0" max="100" value="30"></label><label class="toggle-line"><input id="show-startup-warning" name="show_startup_warning" type="checkbox" checked>Show startup warning</label><label class="toggle-line"><input id="ip-write-guard" name="ip_write_guard" type="checkbox">IP Write Guard</label><label>Geschützte Radio-IPs<input id="protected-device-ips" name="protected_device_ips" placeholder="Radio-IP, weitere Radio-IP"><small>Diese Radios werden vor jedem Netzwerktransport blockiert.</small></label><label>Geschützte Radio-IDs<input id="protected-device-ids" name="protected_device_ids" placeholder="Geräte-ID, weitere Geräte-ID"><small>Die ID-Sperre bleibt auch bei einem DHCP-IP-Wechsel wirksam.</small></label><label>Default timezone<select id="system-timezone-select" name="default_timezone"></select></label><label>Default device language<select id="default-device-language-select" name="device_language_default"></select></label><label class="lab-only">Display metadata mode<select id="display-metadata-mode" name="display_metadata_mode"><option value="off">Off</option><option value="station_clock">Station + clock</option><option value="station_clock_wifi">Station + clock + WiFi</option></select></label><label class="lab-only">First-run warnings<select id="first-run-warning-required" name="first_run_warning_required"><option value="false">Confirmed</option><option value="true">Show again on next open</option></select></label><label class="lab-only">Firmware policy<select name="support_latest_firmware_only"><option value="true">Support latest firmware only</option><option value="false">Allow legacy research mode</option></select></label><label class="lab-only">Supported firmware family<input name="latest_supported_firmware_family" value="27.0.x" readonly aria-readonly="true"></label><div class="firmware-warning lab-only"><strong>⚠ Firmware update und unterstützte Modelle</strong><span>Direkt verifiziert sind nur SoundTouch 10, 20, 30 und Portable.</span></div><button class="command primary" type="submit">Einstellungen speichern</button></form></section>
                <section class="panel"><h3>Notes</h3><pre id="system-settings-output" data-i18n-static>Default UI language is English so every user can reach language setup. Device languages are the Stockholm-supported speaker languages.</pre></section>
              </div>
            </section>
            ...
            <section class="view" id="view-lab">
              ...
              <section class="panel danger-panel lab-only lab-factory-reset-card" id="lab-factory-reset-card">
                <div class="panel-title-row"><div><span class="section-kicker">LAB · destructive</span><h3>Factory Reset Radio</h3></div><span class="status-pill status-warning">LAB ONLY</span></div>
                <p class="muted-copy">This erases the radio configuration and returns the device to factory defaults. BASSWIESN first verifies the current identity, exact firmware profile, and a complete readable backup.</p>
                <form id="lab-factory-reset-form" class="settings-form">
                  <label>Radio<select id="factory-reset-device-select" name="device_id" required></select></label>
                  <div id="factory-reset-identity" class="event-list"><div class="empty">Select a radio to review its identity.</div></div>
                  <label class="toggle-line"><input name="acknowledged" type="checkbox">I understand that this resets the radio</label>
                  <label>Type FACTORY RESET RADIO to confirm<input name="confirmation" autocomplete="off" placeholder="FACTORY RESET RADIO"></label>
                  <div class="button-row"><button class="command" data-factory-reset-action="preview" type="submit">Review backup and reset plan</button><button class="command danger" data-factory-reset-action="execute" type="submit">Factory Reset</button></div>
                </form>
                <div id="factory-reset-status" class="event-list"><div class="empty">No factory reset started.</div></div>
                <details><summary>Backup and technical evidence</summary><pre id="factory-reset-output">No preview loaded.</pre></details>
              </section>
              <div class="split">
